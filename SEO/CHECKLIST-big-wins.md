...
- [x] Rem root (`rem-root.php`) in `<head>` before CSS; Euclid/Bebas use `font-display: optional` (cuts desktop CLS from font swap on `#smooth-content`); desktop-only navbar `min-width` reserve
